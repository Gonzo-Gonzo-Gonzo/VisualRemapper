...
#ethernet left_pi_IP='169.254.95.141'
#ethernet right_pi_IP='169.254.244.232'
#ethernet HOST='169.254.103.155'
...
class displayer():
    ...
    def __init__(self, device_name, Patient_info):
        self.devices={'HTCVive':{'width':2160,'center':1050,'height':1200,'left lense center':[1400,750],'right lense center':[500,750]}}
        ...
        if Patient_info['Hemianopsia_type'] == 'Homonymous left':
            ...
            #MODULARIZE
            self.right_window.geometry(str(self.devices[self.device_name]['width']-self.Patient_info['border_ri'])+'x'+str(self.devices[self.device_name]['height'])+'+'+str(self.devices[self.device_name]['right lense center'][1])+'+0')
            #self.right_imtk=ImageTk.PhotoImage(self.standby_image_pil)
            self.right_img_label=TK.Label(master=self.right_window)
            self.right_img_label.place(x=0,y=0)
            self.left_window=TK.Toplevel()
            #MODULARIZE
            self.left_window.geometry(str(self.devices[self.device_name]['center']-self.patient_info['border_le'])+'x'+str(self.devices[self.device_name]['height'])+'+'+str(self.devices[self.device_name]['left lense center'][1]-100)+'+0')
            #self.left_imtk=ImageTk.PhotoImage(self.standby_image_pil)
            self.left_img_label=TK.Label(master=self.left_window)
            self.left_img_label.place(x=0,y=0)
            self.left_window.update()
            self.right_window.update()
            print ('left_window_width=  '+str(self.left_window.winfo_width()))
            print ('right_window_width= '+str(self.right_window.winfo_width()))
            print ('left_window_height=  '+str(self.left_window.winfo_height()))
            print ('right_window_height= '+str(self.right_window.winfo_height()))
    ...
    def update(self, frames):
        def right_update(frame):    
            ...
            right_imtk= ImageTk.PhotoImage(im_pil)
            self.right_img_label.config(image=right_imtk)
            self.right_window.update()
            
        def left_update(frame):
            im_pil = Image.fromarray(frame)
            left_imtk= ImageTk.PhotoImage(im_pil)
            self.left_img_label.config(image=left_imtk)
            self.left_window.update()
...
class Transformer ():
    width_of_frames=639
    def __init__(self,components,displayer):
    ...
    def transform1(self,frames):#transform 1 adapts the frames that come in from the pi to the HTC_Vive HMD 
        frame_le=frames[0]
        frame_ri=frames[1]
        ...
        print(str(self.displayer.left_window.winfo_width())+' '+str(self.displayer.left_window.winfo_height()))
        frame_le=cv2.resize(src=frame_le,dsize=[self.displayer.left_window.winfo_width(),self.displayer.left_window.winfo_height()])
        print (frame_le.shape)
    ...
    def transform1_1(self,frames,right_window_width,left_window_width,left_window_height,right_window_height):#transform 1 adapts the frames that come in from the pi to the HTC_Vive HMD 
        frame_le=frames[0]
        frame_ri=frames[1]
        result= [] #the list where the images will be put.    
        ...
        #remove the padding
        frame_le=frame_le[0:639,70:440]
        frame_le=cv2.resize(src=frame_le,dsize=[left_window_width,left_window_height])
        print (frame_le.shape)
        ...
        frame_ri=frame_ri[0:639,70:440]
        #resize
        frame_ri=cv2.resize(src=frame_ri,dsize=[right_window_width,right_window_height])
        print (frame_ri.shape)
    ...
    def transform2_10m(self,frames):#Overlap adjuster.
        ...
        S=3
        AFOV=135
        eye_overlap= ((D*2/(math.tan((180-AFOV)/2)))-3)/(3+(D*2/(math.tan((180-AFOV)/2))))
        #calculate the overlap for the cameras with the same distance 
        D=1000
        S=2
        AFOV=70
        camera_overlap=((D*2/(math.tan((180-AFOV)/2)))-3)/(3+(D*2/(math.tan((180-AFOV)/2))))
...
class Remapper():
    def __init__(self,displayer,mode,Info,number_of_frames,transformer):
        self.Info=Info
    ...
    def Stream_Live(self):
        ...
        right_pi_IP= self.Info['Right_pi_IP']
        Left_pi_IP= self.Info['left_pi_IP']
        ...
        while True:
            ...
            frames=self.transformer.transform(frame_le=frame_Le,frame_ri=frame_Ri)
    ...
    def stream_record_one_side(self,number_of_frames,right_or_left):
        result=[]#this list must be filled with tuples where item 0 is the left frame and item 1 is the right frame from the front facing layer. 
        ...
        sender_socket_ri=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        sender_socket_ri.connect((IP,2000))
            #create receiver socket and listen
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.bind((HOST, PORT))#Affix to local machine
        s.listen()
        print('listening')
        conn,addr=s.accept()#accept any connection
        print (addr)
            #send request message and receive the frame
        while len(result)<number_of_frames:
            ...
            #Use recv to receive the message with the frame. 
            #There is a risk that recv returns before receiving a whole frame.This will cause 'pickle.loads' to throw a 'dta truncated exception. 
            while len(frame)<307360:
                frame+=conn.recv(100000) #instruct the receive method to receive something the size of a frame in bytes
            #unpickle (deserialize) the frames.
            try:
